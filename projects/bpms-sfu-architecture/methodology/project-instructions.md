...
The deliverable of this project is the documentation set defined by `bpms-sfu-fpga-design/arch/README.md` (spec layer) and `docs/methodology/01-fpga-project-methodology.md` (execution layer), authored in the order specified below, and committed to `bpms-sfu-fpga-design/arch/`. The legacy working artifact `fpga-arch-framework.md` is superseded by `arch/README.md` and retained only for chat-history continuity.
...
Canonical state is `state.md` in this Project's knowledge base. If anything in these instructions or in chat contradicts it, `state.md` wins.
